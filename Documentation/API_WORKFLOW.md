...
3.  **數值解析**（`value_parsing.py`）：從 Timestream 撈到的是原始字串，會先嘗試轉成數字（`Decimal`）存入 DynamoDB；轉不成數字（例如 GPS 的 `"lat,lng"` 字串）或轉出來是 `NaN`/`Infinity` 的，就原樣以字串儲存，不會被丟棄。
4.  **存儲與 TTL (DynamoDB - SensorDataCache)**：
    *   **分區鍵 (PK)**：`DEVICE#<devID>#DATE#<YYYY-MM-DD>`。
    *   **排序鍵 (SK)**：`SENSOR#<sensorType>#TS#<ISO時間戳>`。
    *   **自動清理 (TTL)**：每筆紀錄包含 `expire_at` 欄位（Unix 時間戳），設定為 **30 天後自動刪除**，以維持資料表性能並節省儲存成本。注意 TTL 是「從寫入當下起算 30 天」，若要匯出「上個月」完整資料，需在當月月初盡快匯出，避免上個月月初的資料已超過 30 天被清除。
...
*   **用途**：
    *   獲取特定設備過去 N 天的特定感測器數據趨勢（例如 Dashboard 的 7/15/30 天重量趨勢圖）。
    *   **RAW 資料匯出**（`RawData.js` 頁面）：一次只能選一種 `sensorType`，前端會計算「上個月」需要回溯的天數，帶入 `days` 參數一次取回，再由前端依時間戳過濾出剛好落在上個月範圍內的紀錄，最後打包成 JSON 檔案下載（不經過任何新後端端點，沿用同一支 API）。
